...
def build_plan_prompt(query: str, catalog_override: str | None = None) -> dict[str, list[dict[str, str]]]:
    ...
    system = (
        ...
        "=== TASK ===\n"
        "Break down the user's image processing request into an ORDERED list of\n"
        "concrete processing steps.\n\n"
        "=== CHAIN-OF-THOUGHT ===\n"
        "Before producing the plan, reason through these steps silently:\n"
        "  1. IDENTIFY the core operations the user wants (blur, resize, blend, etc.).\n"
        "  2. DETERMINE the topology:\n"
        "     - linear: single input → sequential transforms → single output.\n"
        "     - batch:  folder of images → batch_* transforms → batch save.\n"
        "     - branch: multiple inputs that merge (blend/overlay/composite).\n"
        "  3. ORDER the operations logically (input first, output last).\n"
        "  4. VERIFY: Does every step map to exactly one operation from the catalog?\n"
        "     Are input/output bookends present?\n\n"
        "=== AVAILABLE OPERATIONS BY CATEGORY ===\n"
        f"{catalog}\n\n"
        "=== RULES ===\n"
        "1. Every pipeline MUST start with an input (load_image or load_folder).\n"
        "2. Every pipeline MUST end with an output (save_image or batch_save_images).\n"
        "3. Folder processing → load_folder + batch_* operations ONLY. Never mix single-image\n"
        "   filters into a batch chain.\n"
        "4. Compositing two images → two load_image steps + blend or overlay.\n"
        "5. Each step uses exactly ONE operation from the catalog.\n"
        "6. The 'reasoning' field MUST explain WHY you chose this topology and these steps.\n"
        "7. Output ONLY valid JSON. No markdown fences, no surrounding text.\n\n"
        "=== COMMON MISTAKES TO AVOID ===\n"
        "- Using single-image filters (e.g. gaussian_blur) in a batch pipeline instead of batch_gaussian_blur.\n"
        "- Forgetting load_image / save_image bookends.\n"
        "- Setting topology to 'linear' when two images need to merge (should be 'branch').\n"
        "- Inventing operations not in the catalog.\n\n"
        f"=== EXAMPLES ===\n\n{examples_text}"
    )
